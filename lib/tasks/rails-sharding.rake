...
shards_namespace = namespace :shards do
  task _make_activerecord_base_shardable: [:environment] do
    ...
    ActiveRecord::Base.include(Shards::Shardable) unless ActiveRecord::Base.ancestors.include? Shards::Shardable
  end

  desc "Creates database shards (options: RAILS_ENV=x SHARD_GROUP=x SHARD=x)"
  task create: [:environment] do
    Shards::Core.configurations.each do |shard_group, shards_configurations|
  ...
  task migrate: [:_make_activerecord_base_shardable] do
    Shards::Core.configurations.each do |shard_group, shards_configurations|
      ...
      shards_configurations.each do |shard, configuration|
        ...
        Shards::Core.using_shard(shard_group, shard) do
      ...
      Shards::Core.configurations.each do |shard_group, shards_configurations|
        ...
        shards_configurations.each do |shard, configuration|
          ...
          File.open(schema_filename, "w:utf-8") do |file|
            Shards::Core.using_shard(shard_group, shard) do
              ActiveRecord::SchemaDumper.dump(Shards::ConnectionHandler.retrieve_connection(shard_group, shard), file)
            end
          end
        end
      end
  ...
  namespace :test do
    ...
    task load_schema: ['shards:test:purge'] do
      Shards::Core.test_configurations.each do |shard_group, shards_configurations|
        next if ENV["SHARD_GROUP"] && ENV["SHARD_GROUP"] != shard_group.to_s
        ...
        shards_configurations.each do |shard, configuration|
          ...
          begin
            ...
            should_reconnect = Shards::ConnectionHandler.connection_pool(shard_group, shard).active_connection?
            Shards::ConnectionHandler.establish_connection(shard_group, shard, 'test')
            ...
            if should_reconnect
              ...
              Shards::ConnectionHandler.establish_connection(shard_group, shard)
            end
          end
        end
      end
    ...
    task prepare: [:environment] do
      unless Shards::Core.test_configurations.blank?
