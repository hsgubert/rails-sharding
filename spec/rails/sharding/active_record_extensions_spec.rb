...
describe Rails::Sharding::ActiveRecordExtensions do
  ...
  describe '#using_shard method in model classes' do
    it 'should select shard' do
      ...
      expect(Account.using_shard(:mysql_group, :shard1).first).to be_nil
      new_account = Account.using_shard(:mysql_group, :shard1).create
      expect(Account.using_shard(:mysql_group, :shard1).first).to be == new_account

      # checks shard2 has no account
      expect(Account.using_shard(:mysql_group, :shard2).first).to be_nil
    end

    it 'should allow chaining after it is called' do
      new_account = Account.using_shard(:mysql_group, :shard1).create

      expect(Account.using_shard(:mysql_group, :shard1).where(id: new_account.id).first).to be == new_account
      expect(Account.using_shard(:mysql_group, :shard1).where(id: new_account.id + 1).first).to be_nil
    end

    it 'should not accept block' do
      expect {
        Account.using_shard(:mysql_group, :shard1) {}
    ...
    it 'should allow multiple calls, overriding the shard configuration' do
      new_account = Account.using_shard(:mysql_group, :shard1).create
      expect(Account.using_shard(:mysql_group, :shard2).using_shard(:mysql_group, :shard1).where(id: new_account.id).first).to be == new_account
      expect(Account.using_shard(:mysql_group, :shard2).where(id: new_account.id).using_shard(:mysql_group, :shard1).first).to be == new_account
    end

    it 'should be evaluated when compared' do
      new_account = Account.using_shard(:mysql_group, :shard1).create
      another_account = Account.using_shard(:mysql_group, :shard1).create
      expect(Account.using_shard(:mysql_group, :shard1).where(id: new_account.id) == [new_account]).to be true
      expect(Account.where('1=1').using_shard(:mysql_group, :shard1) == [new_account, another_account]).to be true
      expect(Account.using_shard(:mysql_group, :shard1).where(id: new_account.id).eql? [new_account]).to be true
      expect(Account.where('1=1').using_shard(:mysql_group, :shard1).eql? [new_account, another_account]).to be true
    end
  end

  describe '#using_shard method in relations' do
    it 'should allow chaining before it is called' do
      new_user = User.using_shard(:mysql_group, :shard1).create(:username => 'test_username')

      expect(User.where('username="test_username"').using_shard(:mysql_group, :shard1).first).to be == new_user
      expect(User.where('username="other_username"').using_shard(:mysql_group, :shard1).first).to be_nil
    end

    it 'should allow chaining before and after it is called' do
      new_user = User.using_shard(:mysql_group, :shard1).create(:username => 'test_username')

      expect(User.where('username="test_username"').using_shard(:mysql_group, :shard1).where(id: new_user.id).first).to be == new_user
      expect(User.where('username="test_username"').using_shard(:mysql_group, :shard1).where(id: new_user.id + 1).first).to be_nil
    end
  ...
  describe '#using_shard method in model instance' do
    it 'should work before reloading' do
      ...
      new_user.using_shard(:mysql_group, :shard1).reload
