...
describe Rails::Sharding::ShardableModel do
  ...
  describe '.connection_pool' do
    ...
    it 'should retrive sharded connection_pool if using shard' do
      expect(Rails::Sharding::ConnectionHandler).to receive(:connection_pool).twice.with(:shard_group1, :shard1)
  ...
  describe '.retrieve_connection' do
    ...
    it 'should delegate to sharded retrieve_connection if using shard' do
      expect(Rails::Sharding::ConnectionHandler).to receive(:retrieve_connection).once.with(:shard_group1, :shard1)
      Rails::Sharding.using_shard(:shard_group1, :shard1) do
        TestModel.retrieve_connection
      end
    end
  ...
  describe '.connected?' do
    ...
    it 'should delegate to sharded connected? if using shard' do
      expect(Rails::Sharding::ConnectionHandler).to receive(:connected?).once.with(:shard_group1, :shard1)
      Rails::Sharding.using_shard(:shard_group1, :shard1) do
        TestModel.connected?
      end
    end
  ...
  describe '.remove_connection' do
    ...
    it 'should delegate to sharded retrieve_connection if using shard' do
      expect(Rails::Sharding::ConnectionHandler).to receive(:remove_connection).once.with(:shard_group1, :shard1)
      Rails::Sharding.using_shard(:shard_group1, :shard1) do
        TestModel.remove_connection
      end
    end
  ...
  describe '.establish_connection' do
    ...
    it 'should delegate to sharded establish_connection if using shard' do
      expect(Rails::Sharding::ConnectionHandler).to receive(:establish_connection).once.with(:shard_group1, :shard1)
      Rails::Sharding.using_shard(:shard_group1, :shard1) do
        TestModel.establish_connection
      end
    end
